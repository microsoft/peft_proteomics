...
class SymmetryClassificationHead(nn.Module):
...
def get_model(
    ...
    cls_head = SymmetryClassificationHead(
...
def train_epoch(
    ...
    for i, batch in enumerate(train_loader):
        with torch.cuda.amp.autocast(enabled = params.USE_AMP), model.no_sync():
            ...
            feats = feats[:, :params.max_crop].to(rank)
            attn = attn[:, :params.max_crop].to(rank)
            ...
            tm_metrics(active_fn(logits).float().detach(), torch.argmax(labels, dim = -1))
    ...
    for k, v in metrics.items():
        if "perclass" in k:
            for i, v_i in enumerate(v):
                logg.metric(f"{k}_{i}", v_i.item(), step = epoch)
        else:
...
def mp_main(rank, world_size, params):
    ...
    for epoch in range(start_epoch, params.n_epoch):
        ...
        if (epoch + 1) % params.save_every == 0:
            ...
            save_model(f"checkpoint{epoch}", model, optimizer, scheduler, best_valid_loss, best_valid_aupr, epoch, params, logg)
